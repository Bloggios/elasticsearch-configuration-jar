...
@Component
public class SearchFilterBusinessValidator implements BusinessValidator<ListRequest> {

    @Override
    public void validate(ListRequest listRequest) {
        if (Objects.nonNull(listRequest.getField()) && !listRequest.getField().isEmpty()) {
            listRequest
                    .getField()
                    .forEach(searchFilter -> {
                        if (!ValueCheckerUtil.isValuePresent(searchFilter.getField())) {
                            throw new ElasticsearchException(ElasticErrorCodes.SEARCH_FILTER_WITH_EMPTY_FIELD);
                        }
                    });
        }
    }
