...
@Component
public class PageSizeBusinessValidator implements BusinessValidator<ListRequest> {

    @Override
    public void validate(ListRequest listRequest) {
        if (ValueCheckerUtil.isValuesPartialNull(listRequest.getPage(), listRequest.getSize()) || ValueCheckerUtil.isValuesPartialNull(listRequest.getSize(), listRequest.getPage())) {
            throw new ElasticsearchException(ElasticErrorCodes.BOTH_PAGE_SIZE_MANDATORY);
        }
        if (ValueCheckerUtil.isValuesPresent(listRequest.getPage(), listRequest.getSize()) && (listRequest.getPage() + 1) * listRequest.getSize() > 500) {
            throw new ElasticsearchException(ElasticErrorCodes.PAGE_SIZE_LIMIT_EXCEED);
        }
    }
}
