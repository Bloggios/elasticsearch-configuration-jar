...
@Component
public class SearchTextBusinessValidator implements BusinessValidator<ListRequest> {

    @Override
    public void validate(ListRequest listRequest) {
        if (Objects.nonNull(listRequest.getTexts()) && !listRequest.getTexts().isEmpty()) {
            listRequest
                    .getTexts()
                    .forEach(searchText -> {
                        if (!ValueCheckerUtil.isValuePresent(searchText)) {
                            throw new ElasticsearchException(ElasticErrorCodes.SEARCH_TEXT_EMPTY);
                        }
                    });
        }
    }
