...
@Component
public class SearchNGramFilterBusinessValidator implements BusinessValidator<ListRequest> {

    @Override
    public void validate(ListRequest listRequest) {

        if (Objects.nonNull(listRequest.getSearchFieldsNgram()) && !listRequest.getSearchFieldsNgram().isEmpty()) {
            listRequest
                    .getSearchFieldsNgram()
                    .forEach(searchFilterNgram -> {
                        if (!ValueCheckerUtil.isValuePresent(searchFilterNgram.getField())) {
                            throw new ElasticsearchException(ElasticErrorCodes.FIELD_NOT_PRESENT_FOR_SEARCHING_NGRAM);
                        }
                    });
        }
    }
