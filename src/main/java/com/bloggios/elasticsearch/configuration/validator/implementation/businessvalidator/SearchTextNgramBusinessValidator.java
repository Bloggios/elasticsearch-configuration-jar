...
@Component
public class SearchTextNgramBusinessValidator implements BusinessValidator<ListRequest> {

    @Override
    public void validate(ListRequest listRequest) {
        if (Objects.nonNull(listRequest.getSearchTextsNgram()) && !listRequest.getSearchTextsNgram().isEmpty()) {
            listRequest.getSearchTextsNgram().forEach(searchTextsNgram -> {
                if (!ValueCheckerUtil.isValuePresent(searchTextsNgram)) {
                    throw new ElasticsearchException(ElasticErrorCodes.NGRAM_SEARCH_TEXT_EMPTY);
                }
            });
        }
    }
