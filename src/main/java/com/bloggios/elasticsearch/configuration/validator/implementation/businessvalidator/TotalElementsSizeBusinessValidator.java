...
@Component
public class TotalElementsSizeBusinessValidator implements BusinessValidator<ListRequest> {

    @Override
    public void validate(ListRequest listRequest) {
        Integer size = listRequest.getSize();
        if (size != null && size < 1) {
            throw new ElasticsearchException(ElasticErrorCodes.SIZE_NOT_GREATER_THAN_ZERO);
        }
    }
}
